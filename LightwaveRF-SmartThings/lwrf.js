...
function LightwaveRF(callback) {
    ...
    this.timeout = config.timeout || 300; // timeout was originally 1000 but was slow
    this.queue = [];
    this.ready = true;
...
LightwaveRF.prototype.turnRoomMoodOn = function (roomId, moodId, callback) {
    ...
    var state = 1; // State to turn a mood on
...
LightwaveRF.prototype.exec = function() {
    ...
    if (this.queue.length > 40) this.queue = []; //this.queue.clear();
